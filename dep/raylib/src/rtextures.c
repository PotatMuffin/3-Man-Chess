...
Color *LoadImagePalette(Image image, int maxPaletteSize, int *colorCount)
{
    ...
    if (pixels != NULL)
    {
        ...
        for (int i = 0; i < maxPaletteSize; i++) palette[i] = RL_BLANK;   // Set all colors to BLANK
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, mipmapped, faceRecs[i], (Rectangle){ 0, (float)size*i, (float)size, (float)size }, RL_WHITE);
